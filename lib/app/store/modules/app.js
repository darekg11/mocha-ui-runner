...
const getters = {
  ...
  testsGroupedByTestSpec: (state) => _(state.tests).groupBy("file").mapValues((value, key) => ({
    id: key,
    label: key,
    tests: value.map((test) => ({
      id: `${key}-${test.fullTitle}`,
      label: test.fullTitle,
      ...test,
    })),
  })).values()
    .value(),
  ...
  async getInitialData({ commit }) {
    try {
      ...
      Notify.create({
        ...
        icon: "repo      default-expand-allrt_problem",
        position: "top-right",
        timeout: 2500,
      });
